  ...
  <v-container fluid>
    ...
    <v-row>
      <v-col  cols="12" sm="8" v-if="!connection">
        <h2 class="text-h4">New SSH Connection</h2>
      </v-col>
     ...
     <v-col cols="12" sm="8">
      ...
      <v-col cols="12" sm="8" v-if="!connection">
        <v-textarea v-model="privateKey" label="Private key" outlined />
      </v-col>
    </v-row>
...
export default VueApp.extend({
  ...
  computed: {
    ...
    canCreate(): boolean {
      return this.name.length !== 0 && this.privateKey.length !== 0 && this.host.length !== 0;
    },
    canUpdate(): boolean {
  ...
  methods: {
    ...
    async deleteConnection() {
      ...
      if (!confirm('Do you really want to delete this SSH connection?')) {
        return;
      }
