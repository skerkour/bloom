...
pub const SEEDBYTES: usize = libsodium_sys::crypto_sign_ed25519_SEEDBYTES as usize;

/// Number of bytes in a `SecretKey`.
pub const SECRETKEYBYTES: usize = libsodium_sys::crypto_sign_ed25519_SECRETKEYBYTES as usize;

/// Number of bytes in a `PublicKey`.
pub const PUBLICKEYBYTES: usize = libsodium_sys::crypto_sign_ed25519_PUBLICKEYBYTES as usize;

/// Number of bytes in a `Signature`.
pub const SIGNATUREBYTES: usize = libsodium_sys::crypto_sign_ed25519_BYTES as usize;
...
impl SecretKey {
    ...
    pub fn public_key(&self) -> PublicKey {
        ...
        unsafe {
            libsodium_sys::crypto_sign_ed25519_sk_to_pk(pk.0.as_mut_ptr(), self.0.as_ptr());
        }
        pk
    }
}
...
pub fn gen_keypair() -> (PublicKey, SecretKey) {
    ...
    unsafe {
        libsodium_sys::crypto_sign_ed25519_keypair(pk.0.as_mut_ptr(), sk.0.as_mut_ptr());
    }
    (pk, sk)
}
...
/// `sign_combined()` signs a message `m` using the signer's secret key `sk`.
/// `sign_combined()` returns the resulting signed message `sm`.
pub fn sign_combined(m: &[u8], sk: &SecretKey) -> Vec<u8> {
    let mut sm = vec![0u8; m.len() + SIGNATUREBYTES];
    let mut smlen = 0;
    unsafe {
        libsodium_sys::crypto_sign_ed25519(
            sm.as_mut_ptr(),
            &mut smlen,
            m.as_ptr(),
...
/// `verify_combined()` verifies the signature in `sm` using the signer's public key `pk`.
/// `verify_combined()` returns the message `Ok(m)`.
/// If the signature fails verification, `verify()` returns `Err(())`.
pub fn verify_combined(sm: &[u8], pk: &PublicKey) -> Result<Vec<u8>, Error> {
    let mut m = vec![0u8; sm.len()];
    let mut mlen = 0;
    let ret = unsafe {
        libsodium_sys::crypto_sign_ed25519_open(
            m.as_mut_ptr(),
            &mut mlen,
            sm.as_ptr(),
...
/// `sign()` returns the resulting signature `sig`.
...
/// `verify()` verifies the signature in `sig` against the message `m`
/// and the signer's public key `pk`.
/// `verify()` returns true if the signature is valid, false otherwise.
pub fn verify(sig: &Signature, m: &[u8], pk: &PublicKey) -> bool {
    let ret = unsafe {
...
#[derive(Copy, Clone)]
pub struct State(libsodium_sys::crypto_sign_ed25519ph_state);
...
impl State {
    ...
    pub fn init() -> State {
        ...
        let state = unsafe {
            libsodium_sys::crypto_sign_ed25519ph_init(s.as_mut_ptr());
...
mod test {
    ...
    #[test]
    fn test_sign_combined_verify() {
        use crate::rand;

        for i in 0..256usize {
            let (pk, sk) = gen_keypair();
            let m = rand::bytes(i);
            let sm = sign_combined(&m, &sk);
            let m2 = verify_combined(&sm, &pk);
            assert!(Ok(m) == m2);
        }
    }

    #[test]
    fn test_sign_combined_verify_tamper() {
        use crate::rand;

        for i in 0..32usize {
            let (pk, sk) = gen_keypair();
            let m = rand::bytes(i);
            let mut sm = sign_combined(&m, &sk);
            for j in 0..sm.len() {
                sm[j] ^= 0x20;
                assert!(Err(Error::Unknown) == verify_combined(&sm, &pk));
                sm[j] ^= 0x20;
            }
        }
    }
    ...
    fn test_sign_verify_tamper() {
        ...
        for i in 0..32usize {
            let (pk, sk) = gen_keypair();
            let m = rand::bytes(i);
            let Signature(mut sig) = sign(&m, &sk);
            for j in 0..SIGNATUREBYTES {
                sig[j] ^= 0x20;
                assert!(!verify(&Signature(sig), &m, &pk));
                sig[j] ^= 0x20;
            }
        }
    }
    ...
    #[test]
    fn test_sign_combined_verify_seed() {
        use crate::rand;

        for i in 0..256usize {
            let mut seedbuf = [0; 32];
            rand::bytes_into(&mut seedbuf);
    ...
    #[test]
    fn test_sign_combined_verify_tamper_seed() {
        use crate::rand;
...
mod bench {
    ...
    fn bench_sign(b: &mut test::Bencher) {
        ...
        let ms: Vec<Vec<u8>> = BENCH_SIZES.iter().map(|s| rand::bytes(*s)).collect();
        b.iter(|| {
            for m in ms.iter() {
                sign(m, &sk);
            }
