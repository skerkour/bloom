...
pub fn gen_keypair() -> (PublicKey, SecretKey) {
    unsafe {
        ...
        libsodium_sys::crypto_kx_keypair(pk.0.as_mut_ptr(), sk.0.as_mut_ptr());
        (pk, sk)
    }
}
...
pub fn keypair_from_seed(seed: &Seed) -> (PublicKey, SecretKey) {
    unsafe {
        let mut pk = PublicKey([0u8; PUBLICKEYBYTES]);
        let mut sk = SecretKey([0u8; SECRETKEYBYTES]);
        libsodium_sys::crypto_kx_seed_keypair(
            pk.0.as_mut_ptr(),
            sk.0.as_mut_ptr(),
            seed.0.as_ptr(),
        );
...
pub fn server_session_keys(
    ...
    unsafe {
        ...
        let r = libsodium_sys::crypto_kx_server_session_keys(
            rx.0.as_mut_ptr(),
            tx.0.as_mut_ptr(),
            server_pk.0.as_ptr(),
            server_sk.0.as_ptr(),
...
pub fn client_session_keys(
    ...
    unsafe {
        let mut rx = SessionKey([0u8; SESSIONKEYBYTES]);
        let mut tx = SessionKey([0u8; SESSIONKEYBYTES]);
        let r = libsodium_sys::crypto_kx_client_session_keys(
            rx.0.as_mut_ptr(),
            tx.0.as_mut_ptr(),
            client_pk.0.as_ptr(),
            client_sk.0.as_ptr(),
...
mod test {
    ...
    fn test_kx_non_acceptable_keys() {
        ...
        assert!(
            client_session_keys(&client_pk, &client_sk, &fake_server_pk) == Err(Error::Unknown)
        );
        assert!(
            server_session_keys(&server_pk, &server_sk, &fake_client_pk) == Err(Error::Unknown)
        );
    }
