...
pub fn memzero(x: &mut [u8]) {
    unsafe {
        libsodium_sys::sodium_memzero(x.as_mut_ptr() as *mut _, x.len());
    }
}
...
pub fn memcmp(x: &[u8], y: &[u8]) -> bool {
    ...
    unsafe {
        libsodium_sys::sodium_memcmp(x.as_ptr() as *const _, y.as_ptr() as *const _, x.len()) == 0
    }
}
...
pub fn mlock(x: &mut [u8]) -> Result<(), Error> {
    let ret = unsafe { libsodium_sys::sodium_mlock(x.as_mut_ptr() as *mut _, x.len()) };
    if ret == 0 {
        Ok(())
    } else {
        Err(Error::Unknown)
...
pub fn munlock(x: &mut [u8]) -> Result<(), Error> {
    let ret = unsafe {
        // sodium_munlock() internally calls sodium_memzero() to clear memory
        // region.
        libsodium_sys::sodium_munlock(x.as_mut_ptr() as *mut _, x.len())
    };
    if ret == 0 {
        Ok(())
    } else {
...
pub fn increment_le(x: &mut [u8]) {
    unsafe {
        libsodium_sys::sodium_increment(x.as_mut_ptr(), x.len());
    }
}
...
///
/// `add_le()` will return Err<Error> if the length of `x` is not equal to the length of `y`.
///
...
/// If the caller does not do that the cryptographic primitives in crypto42
/// will not uphold any security guarantees (i.e. they may break)
pub fn add_le(x: &mut [u8], y: &[u8]) -> Result<(), Error> {
    if x.len() == y.len() {
        unsafe {
            libsodium_sys::sodium_add(x.as_mut_ptr(), y.as_ptr(), x.len());
        }
        Ok(())
    } else {
        Err(Error::Unknown)
...
mod test {
    ...
    #[test]
    fn test_mlock_fail() {
        // because it bugs in Docker
        if std::env::var("CICD").is_ok() {
            return;
        }
