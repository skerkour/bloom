...
//! ## Algorithm
//!
//! [See here for details](https://libsodium.gitbook.io/doc/secret-key_cryptography/secretstream#algorithm).
//!
//! ## Example
//!
//! ```
//! use crypto42::streaming_aead::xchacha20poly1305::{gen_key, Stream, Tag};
//!
...
//! let key = gen_key();
//! let (mut enc_stream, header) = Stream::init_encrypt(&key).unwrap();
//!
//! // encrypt first message, tagging it as message.
//! let ciphertext1 = enc_stream.encrypt(msg1.as_bytes(), None, Tag::Message).unwrap();
//!
//! // encrypt second message, tagging it as encrypt.
//! let ciphertext2 = enc_stream.encrypt(msg2.as_bytes(), None, Tag::Push).unwrap();
//!
//! // encrypt third message, tagging it as final.
//! let ciphertext3 = enc_stream.encrypt(msg3.as_bytes(), None, Tag::Final).unwrap();
//!
//! // initialize decrypt secret stream
//! let mut dec_stream = Stream::init_decrypt(&header, &key).unwrap();
//!
//! // decrypt first message.
//! assert!(!dec_stream.is_finalized());
//! let (decrypted1, tag1) = dec_stream.decrypt(&ciphertext1, None).unwrap();
//! assert_eq!(tag1, Tag::Message);
//! assert_eq!(msg1.as_bytes(), &decrypted1[..]);
//!
//! // decrypt second message.
//! assert!(!dec_stream.is_finalized());
//! let (decrypted2, tag2) = dec_stream.decrypt(&ciphertext2, None).unwrap();
//! assert_eq!(tag2, Tag::Push);
//! assert_eq!(msg2.as_bytes(), &decrypted2[..]);
//!
//! // decrypt last message.
//! assert!(!dec_stream.is_finalized());
//! let (decrypted3, tag3) = dec_stream.decrypt(&ciphertext3, None).unwrap();
