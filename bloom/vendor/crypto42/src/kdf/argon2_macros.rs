...
impl ToString for HashedPassword {
    fn to_string(&self) -> String {
        unsafe {
            std::str::from_utf8(
                CStr::from_ptr(self.0.as_ptr() as *const _).to_bytes()).expect("error converting HashedPassword to string")
            .to_string()
        }
    }
}
...
impl From<&str> for HashedPassword {
    fn from(string: &str) -> Self {
        ...
        let len = string.len();
        out.0[..len].clone_from_slice(string.as_bytes());
        return out;
    }
}
...
/// `crypto42::init()` once before using any other function from crypto42.
...
/// The `hash_password()` returns a `HashedPassword` which
/// includes:
///
...
/// `verify_password()` verifies that the password `str_` is a valid password
/// verification string (as generated by `hash_password()`) for `passwd`
///
/// It returns `true` if the verification succeeds, and `false` on error.
pub fn verify_password(hp: &HashedPassword, passwd: &[u8]) -> bool {
    let res = unsafe {
