...
pub const DIGEST_512: usize = 64;

pub const DIGEST_256: usize = 32;
...
/// `hash` hashes a message `m`, with an optionnal key `key`. It returns a 512 bits hash `h`.
pub fn hash(m: &[u8], key: Option<&[u8]>) -> Result<Digest, Error> {
    let mut hash_state = State::new(DIGEST_512, key)?;
    hash_state.update(m)?;
...
impl State {
    ...
    pub fn new(out_len: usize, key: Option<&[u8]>) -> Result<State, Error> {
        if out_len < DIGEST_MIN {
            return Err(Error::DigestTooShort);
        } else if out_len > DIGEST_MAX {
            return Err(Error::DigestTooLong);
        }
...
mod test {
    ...
    fn test_blake2b_vectors() {
        ...
        let mut r = BufReader::new(File::open("tests/vectors/hash/blake2b-kat.txt").unwrap());
