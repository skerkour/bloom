...
impl Service {
    pub async fn process_track_event(&self, event: TrackEvent) -> Result<(), kernel::Error> {
        ...
        let mut event_name = event.event_name.trim().to_string();
        event_name.truncate(consts::EVENT_NAME_MAX_LENGTH);

        let properties = event.properties;
        match self.validate_event_properties(&properties) {
            Ok(_) => {}
            Err(_) => return Ok(()),
        };
        ...
        self.repo.create_track_event(&self.db, &event).await?;
